...
export default function EmployeeDashboard({ employee, onLogout }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex flex-col font-sans">
                          ...
                          <td className="p-3 text-center whitespace-nowrap">
                            <div className="flex items-center justify-center space-x-1.5">
                              <button
                                ...
                                className="p-1.5 bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 rounded-lg text-xs font-bold transition cursor-pointer flex items-center space-x-1"
      ...
      {viewTaskModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4" onClick={() => setViewTaskModal(null)}>
          <div className="bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-2xl max-w-lg w-full relative max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <button
            ...
            <div className="flex items-center space-x-3 mb-4 pb-3 border-b border-slate-100">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900">Task Allocation Details</h3>
                <p className="text-xs text-slate-500">For {viewTaskModal.clientName} ({viewTaskModal.date})</p>
              </div>
            </div>

            <div className="space-y-4 overflow-y-auto pr-1 flex-1 text-slate-700 text-xs sm:text-sm">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <span className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider">Client</span>
                  <span className="font-bold text-slate-800">{viewTaskModal.clientName}</span>
                </div>
                <div>
                  <span className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider">Task Type</span>
                  <span className={`inline-flex px-2 py-0.5 text-xs font-bold rounded-full capitalize ${getPostTypeBadgeStyle(viewTaskModal.type)}`}>
                    {viewTaskModal.type}
                  </span>
                </div>
              </div>
              ...
              <div>
                <span className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">Files & Links</span>
                <div className="flex flex-wrap gap-1.5">
                  {viewTaskModal.image && (
                    <button onClick={() => { setViewImageSrc(viewTaskModal.image); setViewTaskModal(null); }} className="px-2.5 py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 rounded-lg text-xs font-bold transition flex items-center space-x-1 cursor-pointer">
                      <span>View Image</span>
                    </button>
                  )}
                  {viewTaskModal.driveUrl && (
                    <a href={viewTaskModal.driveUrl} target="_blank" rel="noreferrer" className="px-2.5 py-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 rounded-lg text-xs font-bold transition flex items-center space-x-1">
                      <span>Drive Link</span>
                      <ExternalLink className="h-3 w-3" />
                    </a>
                  )}
                  {viewTaskModal.mp3Url && (
                    <a href={viewTaskModal.mp3Url} target="_blank" rel="noreferrer" className="px-2.5 py-1 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 rounded-lg text-xs font-bold transition flex items-center space-x-1">
                      <span>MP3 Link</span>
                      <ExternalLink className="h-3 w-3" />
                    </a>
                  )}
                  {viewTaskModal.mp4Url && (
                    <a href={viewTaskModal.mp4Url} target="_blank" rel="noreferrer" className="px-2.5 py-1 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 rounded-lg text-xs font-bold transition flex items-center space-x-1">
                      <span>MP4 Link</span>
                      <ExternalLink className="h-3 w-3" />
                    </a>
                  )}
                  {!viewTaskModal.image && !viewTaskModal.driveUrl && !viewTaskModal.mp3Url && !viewTaskModal.mp4Url && (
                    <span className="text-slate-400 italic text-xs">No files or links provided.</span>
                  )}
                </div>
              </div>

              <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl">
                <span className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Allocator Remark</span>
                <p className="text-slate-700 text-xs leading-relaxed whitespace-pre-wrap font-medium">
                  {viewTaskModal.remark || <span className="text-slate-400 italic font-normal">No remark provided by allocator.</span>}
                </p>
              </div>

              <div className="p-3.5 bg-slate-50/50 border border-slate-200/60 rounded-xl">
                <span className="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1">Work Done Details</span>
                <p className="text-slate-700 text-xs leading-relaxed whitespace-pre-wrap font-medium">
                  {viewTaskModal.workDoneDetails || <span className="text-slate-400 italic font-normal">Pending details / Not started.</span>}
                </p>
              </div>

              {viewTaskModal.hurdlesFaced && (
                <div className="p-3.5 bg-amber-50 border border-amber-200 rounded-xl">
                  <span className="block text-[11px] font-bold text-amber-800 uppercase tracking-wider mb-1">Hurdles & Difficulties Faced</span>
                  <p className="text-amber-950 text-xs leading-relaxed whitespace-pre-wrap font-medium">
                    {viewTaskModal.hurdlesFaced}
                  </p>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-4 pt-3 border-t border-slate-100">
                <div>
                  <span className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Status</span>
                  <span className={`inline-flex px-2 py-0.5 text-xs font-bold rounded capitalize ${
                    viewTaskModal.status === 'completed' ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' :
                    viewTaskModal.status === 'incompleted' ? 'bg-rose-100 text-rose-800 border border-rose-200' :
                    'bg-amber-50 text-amber-700 border border-amber-200'
                  }`}>
                    {viewTaskModal.status}
                  </span>
                </div>
                <div>
                  <span className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">Approval Feedback</span>
                  <div className="flex flex-col space-y-1">
                    <span className={`inline-flex w-fit px-2 py-0.5 text-[10px] font-extrabold rounded uppercase ${
                      viewTaskModal.approvalStatus === 'approved' ? 'bg-emerald-100 text-emerald-800 border border-emerald-250' :
                      viewTaskModal.approvalStatus === 'disapproved' ? 'bg-rose-100 text-rose-800 border border-rose-250' :
                      'bg-slate-100 text-slate-700 border border-slate-200'
                    }`}>
                      {viewTaskModal.approvalStatus || 'pending'}
                    </span>
                    {viewTaskModal.approvalRemark && (
                      <span className="text-[11px] text-slate-500 italic block">{viewTaskModal.approvalRemark}</span>
                    )}
                  </div>
                </div>
              </div>
